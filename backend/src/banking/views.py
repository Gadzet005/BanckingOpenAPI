    ...
    def post(self, request, *args, **kwargs):
        ...
        if event_type == "transaction":
            ...
            if not all([bank_code, account_code, amount, date_str, balance, subtype]):
                return Response(
                    {
                        "error": f"Нет всех обязательных полей {[bank_code, account_code, amount, date_str, balance, subtype]}"
                    },
            ...
            try:
                ...
                return Response(
                    {"error": "Банк не найден"}, status=status.HTTP_404_NOT_FOUND
                )
            ...
            try:
                ...
                return Response(
                    {"error": "Счет не найден"}, status=status.HTTP_404_NOT_FOUND
                )
            ...
            return Response(
                {"status": "Транзакция создана", "transaction_id": transaction.id},
            ...
            if not all(
                ...
                return Response(
                    {
                        "error": f"Нет всех обязательных полей {[amount, period, period_type, date_str, account_code, bank_code, creator]}"
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            return Response(
                {
                    "status": "Периодический платеж создан",
                    "periodic_payment_id": payment.id,
                },
                status=status.HTTP_201_CREATED,
            )
    ...
    def post(self, request):
        ...
        try:
            account = Account.objects.create(
                bank_id=bank, account_code=account_code, user_id=user
            )
        except:
            return Response(status=status.HTTP_400_BAD_REQUEST)
